...
interface ModalProps {
    ...
    title?: string;
    showHeader?: boolean;
    size?: 'normal' | 'large'; // Agrega los valores de tamaño que necesites
}


const Modal = ({ children, show, setShow, title = 'Modal', showHeader = true, size = 'large' }: ModalProps) => {
        ...
        <>
            {show && (
                <div className="fixed top-0 left-0 z-[999] w-screen h-screen bg-black bg-opacity-50 flex justify-center items-center">
                    <div className={` ${size === 'normal' && 'w-10/12 md:w-[35rem]'} ${size === 'large' && 'w-11/12 md:w-[45rem]'}  min-h-[5rem] bg-white opacity-100 relative rounded-md shadow-xl p-4`}>
                        {
                            showHeader && (
